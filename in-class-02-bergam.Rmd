...
names(rain.df) <- c("year","month","day",seq(0,23))
head(rain.df)
#The last 24 columns are hours of each day
```
...
```{r}
#Rainfall sum for each day
library(dplyr)
daily.rain<-mutate(rain.df, daily = rowSums(rain.df[,4:27]))


hist(daily.rain$daily)
...
```{r}
daily.rain[daily.rain < 0] = NA

rain.df.fixed <- daily.rain
